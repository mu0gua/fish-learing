...
print(x_padding[0])
word2vec_tool.embedding_sentences(x_padding, embedding_size = FLAGS.embedding_size, file_to_save = os.path.join(wVec_dir, 'trained_word2vec.model'))
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        #optimizer= tf.optimizers.Adam(1e-3)  # 优化算法
        optimizer= tf.compat.v1.train.AdamOptimizer(1e-3)
	#optimizer= tf.keras.optimizers.Adam(1e-3)  # 优化算法
        ...
        # 正确率与损失率
        loss_summary = tf.compat.v1.summary.scalar("loss", text_cnn.loss)
        acc_summary = tf.compat.v1.summary.scalar("accuracy", text_cnn.accuracy)
